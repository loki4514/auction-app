...
export class UserUpdateService {
    ...
    async updateUser(
        ...
        try {
            ...
            if (!updatedCount) {
                this.logger.log(`No user found with ID: ${user_id}`);
                return {
            ...
            this.logger.log(`User with ID ${user_id} updated successfully`);
            return {
                status: HttpStatus.OK,
                update_status: true,
            };
        } catch (error) {
            this.logger.error(error, `Failed to update user with ID: ${user_id}`);
            return {
